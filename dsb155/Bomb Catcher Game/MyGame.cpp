/*
	...
	Desc: A simple game where the user collects bombs using a bucket.
		  If a bomb falls to the bottom of the screen, the player fails.
*/
...
void Game_Run(HWND window)
{
	...
	if ( SUCCEEDED( d3ddev->BeginScene() ) )
	{
		...
		for(int i = 0; i < ((sizeof(bombs) / sizeof(*bombs))); i++)
		{
			DrawSurface(backbuffer, bombs[i].x, bombs[i].y, bomb_surf);
		}
